...
test("CommonLabel - 単一アイテム リグレッションテスト", async () => {
  ...
  await expect(page.getByTestId("common-label-root")).toMatchScreenshot("common-label-single")
})
...
test("CommonLabel - 複数アイテム リグレッションテスト", async () => {
  ...
  // スクリーンショット比較
  await expect(page.getByTestId("common-label-root")).toMatchScreenshot("common-label-multiple")
})
