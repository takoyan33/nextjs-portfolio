...
describe("HomeSkills コンポーネント", () => {
  test("正常系: スキルが描画される", async () => {
    vi.mocked(fetchFrontSkills).mockResolvedValue({
      status: "SUCCESS",
      data: [{ id: 1, name: "HTML", rank: "A", icon: "", about: "", tag: "aaa" }],
    })
    vi.mocked(fetchBackSkills).mockResolvedValue({
      status: "SUCCESS",
      data: [{ id: 1, name: "PHP", rank: "B", icon: "", about: "", tag: "bbb" }],
    })
    vi.mocked(fetchInfraSkills).mockResolvedValue({
      status: "SUCCESS",
      data: [{ id: 1, name: "AWS", rank: "B", icon: "", about: "", tag: "ccc" }],
    })
    vi.mocked(fetchOtherSkills).mockResolvedValue({
      status: "SUCCESS",
  ...
  test("空データ: スキルが存在しない場合は描画されない", async () => {
    vi.mocked(fetchFrontSkills).mockResolvedValue({ status: 404, data: [] })
    vi.mocked(fetchBackSkills).mockResolvedValue({ status: 404, data: [] })
    vi.mocked(fetchInfraSkills).mockResolvedValue({ status: 404, data: [] })
    vi.mocked(fetchOtherSkills).mockResolvedValue({ status: 404, data: [] })
  ...
  test("異常系: fetchが例外を投げる場合は呼び出し側でエラーになる", async () => {
    ...
    // 他は成功しておく
    vi.mocked(fetchBackSkills).mockResolvedValue({ status: "SUCCESS", data: [] })
    vi.mocked(fetchInfraSkills).mockResolvedValue({ status: "SUCCESS", data: [] })
    vi.mocked(fetchOtherSkills).mockResolvedValue({ status: "SUCCESS", data: [] })
