...
describe("ContactFormの表示とバリデーション", () => {
  ...
  test("必須項目が空の場合はエラーメッセージが表示される", async () => {
    ...
    expect(screen.getByText("名前は入力必須です")).toBeVisible()
    expect(screen.getByText("正しいメールアドレスを入力してください")).toBeVisible()
    expect(screen.getByText("問い合わせ内容は5文字以上必要です")).toBeVisible()
  })
})
