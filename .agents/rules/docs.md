...
- パッケージマネジャーはpnpmを利用
 - ディスク容量の効率化
 - デプロイ時間の短縮(yarnの時より1分30秒ほど早い。)
   -  yarnは依存解決に時間がかかる
   -  pnpmはsymbolic link/symlink ベース
   -  pnpm は並列ダウンロード
---
...
| hover | ポートフォリオカードにマウスオーバー | 画像のスケールアニメーション | 画像が1.1倍に拡大 |
| change | フィルターセレクトボックス変更 | 選択されたタグでフィルタリング | 該当するポートフォリオのみ表示 |

## 7. APIインターフェース
| エンドポイント | メソッド | リクエスト例 | レスポンス例 |
|:---|:---|:---|:---|
| /api/portfolios/:id | GET | `{ id: 1 }` | `{ id: 1, name: "React Portfolio", date: "2025-01-01", tag: ["React"], topImg: "/images/portfolio.png" }` |
| /api/portfolios | GET | `{}` | `[{ id: 1, name: "Portfolio 1" }, ...]` |
